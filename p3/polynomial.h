...
class SparsePolynomial : public sparse_vector_t {
 public:
  ...
  bool IsEqual(const Polynomial&, const double = EPS) const;
  void Modificacion(double i) const;
};
...
bool Polynomial::IsEqual(const Polynomial& pol, const double eps) const {
  ...
  for (int i{0}; i < size; i++) {
    if (IsNotZero(at(i) - pol.at(i))) return false;
  }
  return !differents;
}
...
bool SparsePolynomial::IsEqual(const SparsePolynomial& spol
  ...
  // poner el código aquí
  for (int i{0}; i < get_nz(); i++) {
    if (abs(at(i).get_val() - spol.at(i).get_val()) > eps) return false;
  }
  return !differents;
}
...
bool SparsePolynomial::IsEqual(const Polynomial& pol, const double eps) const {
  bool differents = false;
  // poner el código aquí
  SparsePolynomial a(pol);
  for (int i{0}; i < get_nz(); i++) {
    if (abs(at(i).get_val() - a.at(i).get_val()) > eps) return false;
  }
  return !differents;
}

// MODIFICACIÓN
void SparsePolynomial::Modificacion(double i) const {
  bool a = false;
  for (int j = 0; j < get_nz(); j++) {
    if (at(j).get_inx() == i) {
      std::cout << at(j).get_val() << " ";
      a = true;
    }
  }
  if (a == false) {
