...
maze_t::solve()
{
  if (!is_ok_(i_start_, j_start_)) return false;
...
{ 
  ...
  // - la celda (i, j) no puede haber sido visitada antes.
    cout << i << " " << j << endl;
  if (i > 0 && j > 0 && i <= matrix_.get_m() && j <= matrix_.get_n() && matrix_(i,j) != WALL_ID && visited_(i, j) != true ) {
    return true;
  }
  return false;
}
